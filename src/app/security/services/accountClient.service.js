(function () {
  ...
  angular.module('skiptrace.security')
    .factory('accountClientSvc', accountClientSvc);

  accountClientSvc.inject = ['$q', '$http', '$settings'];
  /* @ngInject */
  function accountClientSvc($q, $http, $settings) {

    // Routes
    var baseUrl = $settings.settings.apiBaseUrl.value,
      apiUrl = baseUrl + "/api/account",
    ...
    function getExternalLogins(returnUrl, generateState) {
      ...
      var url = externalLoginUrl + "?returnUrl=" + (encodeURIComponent(baseUrl + '/' + returnUrl)) +
        "&generateState=" + (generateState ? "true" : "false");
      return $http({
          method: 'GET',
          url: url
